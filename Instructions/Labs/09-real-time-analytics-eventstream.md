...
事件流是 Microsoft Fabric 中的一项功能，可捕获、转换实时事件并将其路由到各种目标，并且无需编写代码。 在门户中创建 Eventstream 项时，它是 Fabric 事件流的实例。 可向事件流添加事件数据源、路由目标以及事件处理程序（如果需要转换）。 Azure Service Fabric 的 EventStore 是一个监视选项，用于维护群集中的事件，并提供一种方法来了解群集或工作负载在给定时间点的状态。 可以向 EventStore 服务查询可用于群集中的每个实体和实体类型的事件。 这意味着可在群集、节点、应用程序、服务、分区和分区副本等不同级别查询事件。 EventStore 服务还能够将群集中的事件相关联。 通过查看在同一时间从可能已相互影响的不同实体写入的事件，EventStore 服务能够将这些事件进行关联来帮助查明群集中发生各项活动的原因。 另一种监视和诊断 Azure Service Fabric 群集的方法是使用 EventFlow 聚合和收集事件。
...
借助 Microsoft Fabric 事件流，可在一个位置轻松管理事件数据。 可收集、转换实时事件数据，并以所需格式将其发送到不同目标。 还可毫不费力地将事件流与 Azure 事件中心、KQL 数据库和 Lakehouse 相连接。

本实验室基于名为“股市数据”的示例流式处理数据。 股市示例数据是一个股票交易所的数据集，其中预设了时间、符号、价格、成交量等架构列。 你将使用此示例数据模拟股票价格的实时事件，并使用各种目标（如 KQL 数据库）对其进行分析。

使用实时分析的流式处理和查询功能回答有关股票统计信息的关键问题，并能够使用其结果生成 Power BI 报表。 在此场景中，我们将充分利用向导，而不是单独手动创建某些组件，如 KQL 数据库。
...
接下来将在 OneLake 中启用可用性

1. 在“数据库详细信息”面板中，选择铅笔图标。
...
3. 系统会提示“为 Eventstream 命名”

![为事件流命名](./Images/name-eventstream.png)

4. 为 EventStream 指定一个你能记住的名称（如 *MyStockES），然后按“创建”按钮 。
...
1. 在 Eventstream 画布中，选择“新建目标”，然后选择“KQL 数据库” 

![EventStream 目标](./Images/new-kql-destination.png)

2. 在 KQL 数据库配置中，使用下表完成配置。
...
1. 在本练习中，你已创建 KQL 数据库并使用 EventStream 设置了持续流式处理。 然后使用 KQL 和 SQL 查询了数据。
2. 如果已完成 KQL 数据库探索，可删除为本练习创建的工作区。 
3. 在左侧栏中，选择你的工作区的图标。
4. 在工具栏上的“...”菜单中，选择“工作区设置”。
5. 在“其他”部分中，选择“删除此工作区”。