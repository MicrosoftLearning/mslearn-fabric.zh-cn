...
> **重要**：此练习已弃用，不久将被删除或取消有效性。 说明不再准确，不支持练习。
...
1. 在 [Microsoft Fabric 主页](https://app.fabric.microsoft.com/home?experience=fabric) (`https://app.fabric.microsoft.com/home?experience=fabric`) 上，选择“**Data Activator**”。
...
如果你当前未进入“数据”模式，请选择屏幕左下角的“数据”选项卡******。 在现实示例中，你可以在此处通过 EventStreams 和 Power BI 视觉对象添加自己的数据源。 对于本实验，你将使用 Data Activator 提供的示例数据。 对于此示例，已经为其设置了三个用于监视包裹交付状态的 EventStream。
...
1. 现在需要输入条件的值。 输入 33 和 41 作为范围值************。 由于选择了“退出数字范围”条件，因此当温度低于 33 度或高于 41 度时，触发器应会触发******。
...
1. 由于更新了触发器，正确的操作应该是更新而不是保存触发器，但对于本实验，我们将执行相反的操作，因此请选择“保存”按钮而不是“更新”按钮来看看会发生什么情况********。 选择“更新”按钮的原因是，当你选择更新触发器时，它会保存触发器并使用新条件更新当前运行的触发器****。 如果只是选择了“保存”按钮，则当前运行的触发器不会应用新条件，直到你选择更新触发器**。 让我们继续选择“保存”按钮****。

1. 因为你选择了“保存”而非“更新”，所以你看到消息“存在可用的属性更新。****** 请立即更新以确保触发器具有最新的更改”出现在屏幕顶部。 该消息还附带了一个“更新”按钮**。 让我们继续选择“更新”按钮****。
