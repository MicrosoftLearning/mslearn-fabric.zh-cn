...
使用 Microsoft Fabric 中的实时仪表板，可以使用 Kusto 查询语言 (KQL) 可视化和浏览流数据。  在本练习中，你将了解如何基于实时数据源创建和使用实时仪表板。
...
1. 运行查询，该查询显示过去 30 分钟内每个街区观察到的自行车数量和空自行车停靠点。
...
1. 运行查询，该查询显示过去 30 分钟内每个街区观察到的自行车位置和数量。
1. 应用更改以查看仪表板磁贴中表中显示的数据。
1. 在磁贴上，选择“**编辑**”图标（看起来像铅笔）。 然后在“**视觉格式设置**”窗格中，设置以下属性：
    - **磁贴名称**：自行车位置
    - **视觉对象类型**：地图
...
1. 应用更改，然后调整地图图块的大小，以填充仪表板右侧的可用空间：
...
1. 选择“完成”以创建参数。
    ...
    添加参数后，需要修改基本查询，以基于所选邻里筛选数据。

1. 在工具栏上，选择“**基本查询**”。 然后选择“**base_bike_data**”查询，并对其进行编辑，在 **where** 子句中添加 **and** 条件，以便根据所选参数值进行筛选，如下代码所示：
...
1. 应用更改。 然后调整磁贴的大小，以填充仪表板的高度。

   ![包含两个页面的仪表板的屏幕截图](./Images/dashboard-page-2.png)
